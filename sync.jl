...
const posts = [
    # NEWEST TO OLDESTS
	Post("game of 24 journey", "2025/11/03", "Solving the Game of 24", [], "Going through my process of writing a solver for the Game of 24."),
...
function process_file(file_contents)
    ...
    function tikzsrc2imgsrc(m)
        ...
        outpath, io = mktemp()
        close(io)
        run(pipeline(inpath, `pwrap pdflatex -interaction batchmode -halt-on-error file.tex tex pdf`, `magick -density 300 pdf:- png:-`, `pwrap mogrify -strip file.png png png`, `base64`, outpath))
